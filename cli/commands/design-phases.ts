...
import { hasTopics, readDesignData, saveDesignData, type Phase } from "../lib/design";
...
function render(phases: Phase[]): string {
  return phases.map((p) => `${p.id}: ${p.name}\n  goal: ${p.goal}`).join("\n\n");
}

export async function designPhasesCommand(slug: string): Promise<void> {
  ...
  const phases = await reviewLoop(
    (feedback) =>
      runClaudeTaskJson<Phase[]>(
        buildPrompt(project.objective, project.targetAudience, project.platforms, project.campaignDays, feedback),
        projectDir(slug)
      ),
    render
  );
