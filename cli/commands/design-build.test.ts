...
import { readDesignData, saveDesignData, type ContentStructure, type DesignData } from "../lib/design";
    ...
    phases: [
      {
        ...
        name: "Phase One",
        goal: "g",
        topics: [{ id: "topic-a", title: "Generic Topic A", contentStructures }],
      },
    ],
  };
}
...
describe("designBuildCommand", () => {
  ...
  beforeEach(() => {
    ...
    reviewLoopMock.mockImplementation(
      async (generate: (feedback?: string) => unknown, render: (result: unknown) => string) => {
  ...
  it("exits if the project has no template set", async () => {
    ...
    readDesignDataMock.mockReturnValue(makeDesign([{ variant: "a", hook: "h", bridge: "b", content: "c", cta: "cta" }]));

    await expect(designBuildCommand(slug, "topic-a")).rejects.toThrow("process.exit(1)");
    expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining("No template set"));
  ...
  it("exits if the topic doesn't exist", async () => {
    ...
    await expect(designBuildCommand(slug, "does-not-exist")).rejects.toThrow("process.exit(1)");
  ...
  it("exits if the topic has no locked content structure yet", async () => {
    ...
    await expect(designBuildCommand(slug, "topic-a")).rejects.toThrow("process.exit(1)");
    expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining("no locked content structure"));
  });
  ...
  it("exits if the topic has more than one locked content structure", async () => {
    ...
    await expect(designBuildCommand(slug, "topic-a")).rejects.toThrow("process.exit(1)");
    expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining("needs exactly one locked variant"));
  });
  ...
  it("exits if the locked content structure has no edit copy yet", async () => {
    ...
    await expect(designBuildCommand(slug, "topic-a")).rejects.toThrow("process.exit(1)");
    expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining("no locked edit copy"));
  });

  it("builds a prompt with the locked copy, editCopy span, and hard extraction constraints", async () => {
    readProjectDataMock.mockReturnValue(makeProject({ template: "default" }));
    readDesignDataMock.mockReturnValue(
      makeDesign([{ variant: "a", hook: "MY HOOK", bridge: "b", content: "c", cta: "cta", editCopy: lockedEditCopy }])
    );
    ...
    expect(prompt).toContain("never reuse anything under a .frame-check/ folder");
    expect(prompt).toContain("Full native resolution, no scale flag");
    expect(prompt).toContain("Check src/test-project/ for any composition already built");
    expect(prompt).toContain("public/Projects/test-project/Assets/Video/topic-a.mp4");
  });

  it("doesn't hardcode a reference to any specific private project's composition", async () => {
    readProjectDataMock.mockReturnValue(makeProject({ template: "default" }));
    readDesignDataMock.mockReturnValue(
      makeDesign([{ variant: "a", hook: "h", bridge: "b", content: "c", cta: "cta", editCopy: lockedEditCopy }])
    );
    runClaudeTaskJsonMock.mockReturnValue(makeProposal());
  ...
  it("tells the model to generate a hook still for a template that has one ('default')", async () => {
    ...
    expect(prompt).toContain("public/Projects/test-project/Assets/Images/topic-aHookBG.jpg");
  });
  ...
  it("tells the model NOT to generate a hook still for a template with none ('3-beats')", async () => {
    ...
    runClaudeTaskJsonMock.mockReturnValue(makeProposal({ hookStill: null }));
  ...
  it("saves the returned build output onto the topic's content structure", async () => {
    readProjectDataMock.mockReturnValue(makeProject({ template: "default" }));
    const design = makeDesign([{ variant: "a", hook: "h", bridge: "b", content: "c", cta: "cta", editCopy: lockedEditCopy }]);
    readDesignDataMock.mockReturnValue(design);
    const proposal = makeProposal();
    runClaudeTaskJsonMock.mockReturnValue(proposal);

    await designBuildCommand(slug, "topic-a");

    expect(saveDesignDataMock).toHaveBeenCalledTimes(1);
    const savedDesign = saveDesignDataMock.mock.calls[0][1] as DesignData;
    expect(savedDesign.phases[0].topics![0].contentStructures![0].build).toEqual({
  ...
  it("warns (but doesn't block) when the command's own file check finds a path the agent claimed but didn't write", async () => {
    ...
    expect(renderedOutput).toContain("WARNING: command-level check found a missing file");
  ...
  it("confirms all files exist when the command's own check agrees with the agent", async () => {
    ...
    const renderedOutput = logSpy.mock.calls.map((call) => call[0]).join("\n");
    expect(renderedOutput).toContain("Command-level check: all referenced files exist on disk.");
  });

  it("bounds self-verification to a spot-check and forbids re-deriving editCopy's crop math", async () => {
    readProjectDataMock.mockReturnValue(makeProject({ template: "default" }));
    readDesignDataMock.mockReturnValue(
      makeDesign([{ variant: "a", hook: "h", bridge: "b", content: "c", cta: "cta", editCopy: lockedEditCopy }])
    );
    ...
    expect(prompt).toContain("Extract 2-3 frames TOTAL");
    expect(prompt).toContain("do not repeat the frame-checking edit-copy already did");
    expect(prompt).toContain("editCopy's objectPosition/effect values are already-verified ground truth");
  });
});
