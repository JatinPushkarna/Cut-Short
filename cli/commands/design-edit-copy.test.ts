...
function makeDesign(contentStructures: ContentStructure[] = [{ variant: "a", hook: "h", bridge: "b", content: "c", cta: "cta" }]): DesignData {
  return {
    phases: [
      {
        id: "phase-1",
...
describe("designEditCopyCommand", () => {
  ...
  beforeEach(() => {
    ...
    // given, so runClaudeTaskJson gets called and its prompt is inspectable.
  ...
  it("exits if the project has no template set", async () => {
    ...
    await expect(designEditCopyCommand(slug, "topic-a")).rejects.toThrow("process.exit(1)");
  ...
  it("exits if the topic doesn't exist", async () => {
    ...
    await expect(designEditCopyCommand(slug, "does-not-exist")).rejects.toThrow("process.exit(1)");
  ...
  it("reads the real 'default' template manifest and reflects its beat structure in the prompt", async () => {
    ...
    expect(prompt).toContain("separate still/black beat before any footage plays");
    expect(prompt).not.toContain("overlays hook text on real footage from frame 0 -- confirm");
  });

  it("does not redraft copy or reconsider the template -- both passed as fixed inputs", async () => {
    readProjectDataMock.mockReturnValue(makeProject({ template: "default" }));
    readDesignDataMock.mockReturnValue(
      makeDesign([{ variant: "a", hook: "MY LOCKED HOOK", bridge: "MY LOCKED BRIDGE", content: "MY LOCKED CONTENT", cta: "cta" }])
    );
    runClaudeTaskJsonMock.mockReturnValue({ editCopy: { sourceVideo: "/video.mp4", rows: [] } });
  ...
  it("saves the returned editCopy onto the topic's single locked content structure", async () => {
    ...
    const editCopy = { sourceVideo: "/video.mp4", rows: [{ timestamp: "0:01", action: "CUT IN" }] };
    runClaudeTaskJsonMock.mockReturnValue({ editCopy });
