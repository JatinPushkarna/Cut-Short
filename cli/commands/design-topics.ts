import { hasContentStructures, readDesignData, saveDesignData, type Phase } from "../lib/design";
...
function buildPrompt(phases: Phase[], feedback?: string): string {
  const lines: string[] = [];
  lines.push("You are proposing topics for an already-approved campaign phase schedule.");
  lines.push("");
  lines.push(
  ...
  lines.push(JSON.stringify(phases.map(({ id, name, goal }) => ({ id, name, goal })), null, 2));
  lines.push("");
  lines.push(
...
type TopicsByPhase = {
  phaseId: string;
  topics: { id: string; title: string; description: string; reasoning: string }[];
}[];
...
export async function designTopicsCommand(slug: string): Promise<void> {
  ...
  const topicsByPhase = await reviewLoop(
    (feedback) => runClaudeTaskJson<TopicsByPhase>(buildPrompt(design.phases, feedback), projectDir(slug)),
    render
  );

  const byId = new Map(topicsByPhase.map((entry) => [entry.phaseId, entry.topics]));
