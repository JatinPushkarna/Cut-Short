...
- `cutshort render <slug> --topic <id>` — mechanical Remotion render of a built topic to `Rendered/<topicId>.mp4`; runs automatically after `build --finalize` unless `--skip-render` is passed, so this is normally only needed to re-render an already-finalized topic
